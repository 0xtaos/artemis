...
impl<M: Middleware + 'static> OpenseaSudoArb<M> {
    ...
    async fn process_new_block_event(&mut self, event: NewBlock) -> Result<()> {
        ...
        let new_pools = self
            .get_new_pools(event.block.number.unwrap().as_u64(), event.block.number.unwrap().as_u64())
            .await?;
        // Find existing pools that were touched in the last block.
        let touched_pools = self
            .get_touched_pools(event.block.number.unwrap().as_u64(), event.block.number.unwrap().as_u64())
            .await?;
